...
public class ConfigScreen extends Screen {
    ...
    private void setDefaultPosition() {
        ...
        ModConfig.hudAnchorX = 1;
        ModConfig.hudAnchorY = 2;
        ModConfig.hudOffsetX = 0;
        ModConfig.hudOffsetY = 68 - boxHeight;
        ModConfig.hudX = ModConfig.getHudX(this.width, boxWidth);
        ModConfig.hudY = ModConfig.getHudY(this.height, boxHeight);
        ModConfig.anchorPositionConfigured = true;
    }
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        ModConfig.initializeAnchoredPosition(this.width, this.height, boxWidth, boxHeight);
        ...
        if (this.isDragging) {
            ...
            snappedX = Math.max(0, Math.min(snappedX, this.width - boxWidth));
            snappedY = Math.max(0, Math.min(snappedY, this.height - boxHeight));
            ModConfig.setAnchoredPosition(snappedX, snappedY, this.width, this.height, boxWidth, boxHeight);
        } else {
            this.showVerticalGuide = false;
            this.showHorizontalGuide = false;
        }

        int x = ModConfig.getHudX(this.width, boxWidth);
        int y = ModConfig.getHudY(this.height, boxHeight);
